...
uint8_t Receive_finish;
uint8_t Transceive_finish;

uint32_t ProtocolValue;
uint8_t HID_GetReport_Value[2];
uint8_t HID_SetReport_Value[3];
...
#if 0
ONE_DESCRIPTOR CustomHID_Mouse_Report_Descriptor =
  {
    (uint8_t *)CustomHID_Mouse_ReportDescriptor,
    CUSTOMHID_SIZ_MOUSE_REPORT_DESC
  };
#endif
...
ONE_DESCRIPTOR CustomHID_Hid_Descriptor1 =
  {
    (uint8_t*)CustomHID_ConfigDescriptor + CUSTOMHID_OFF_HID_DESC1,
    CUSTOMHID_SIZ_HID_DESC
  };


ONE_DESCRIPTOR CustomHID_Hid_Descriptor2 =
  {
    (uint8_t*)CustomHID_ConfigDescriptor + CUSTOMHID_OFF_HID_DESC2,
    CUSTOMHID_SIZ_HID_DESC
  };
#endif
...
//#else ---
  ...
  /* Initialize Endpoint 1 */
	SetEPType(ENDP1, EP_INTERRUPT);
  	...
  	SetEPTxCount(ENDP1, EP1_TXBUF);
 	 SetEPRxCount(ENDP1, EP1_RXBUF);
...
RESULT CustomHID_Data_Setup(uint8_t  RequestNo)
{
  ...
  {
  		if(pInformation->USBwValue1 == HID_FEATURE)// && !GetReportFlag)          // report type
        {
        ...
        && RequestNo == SET_REPORT)
  {
  		...
  		if(pInformation->USBwValue1 == HID_FEATURE)          // report type
        {
...
/*******************************************************************************
* Function Name  : CustomHID_GetMouseReportDescriptor.
* Description    : Gets the HID report descriptor.
* Input          : Length
* Output         : None.
* Return         : The address of the configuration descriptor.
*******************************************************************************/
#if 0
uint8_t *CustomHID_GetMouseReportDescriptor(uint16_t Length)
{
  return Standard_GetDescriptorData(Length, &CustomHID_Mouse_Report_Descriptor);
}
#endif
/*******************************************************************************
...
#if 0
uint8_t *CustomHID_GetHIDDescriptor1(uint16_t Length)
{
	//getreportcnt ++;
  return Standard_GetDescriptorData(Length, &CustomHID_Hid_Descriptor1);
}


uint8_t *CustomHID_GetHIDDescriptor2(uint16_t Length)
{
	//getreportcnt ++;
  return Standard_GetDescriptorData(Length, &CustomHID_Hid_Descriptor2);
}
#endif
...
{
  //uint32_t  wOffset;
  //getreportcnt = pInformation->Ctrl_Info.Usb_wOffset;
  if (Length == 0)
  {
 ...
 //GetReportFlag = 1;
 //getreportcnt ++;
  HID_GetReport_Value[0] = pInformation->USBwValue0;
  HID_GetReport_Value[1] = TOUCH_DATA_COUNT;//0x02;
...
{
  //uint32_t  wOffset;
  //setreportcnt = pInformation->Ctrl_Info.Usb_wOffset;

  if (Length == 0)
  {
    pInformation->Ctrl_Info.Usb_wLength = 3;     //2
    return 0;
  }
  
  return (HID_SetReport_Value);  //04 02 78 3bytes data
}
#endif  /*  #if 0 -*/
